            ...
            <div className="flex gap-1 md:gap-3 flex-1">
              {product.variations?.map((variation, index) => (
                  <button
                      ...
                      className={`px-2 md:px-4 py-2 border rounded-lg text-black text-xs flex-1 min-w-0 ${
                          selectedColor === variation.color.toLowerCase()
            ...
            <div className="flex gap-1 md:gap-3 flex-1">
              {getUniqueSizes(getAvailableSizes()).map((sizeObj, index) => (
                  <button
                      ...
                      className={`px-2 md:px-4 py-2 border rounded-lg text-black text-xs flex-1 min-w-0 ${
                          selectedSize === sizeObj.size ? 'border-gray-400 bg-gray-100' : 'border-gray-200'
                      }`}
                  >
                    {sizeObj.size}
                  </button>
              ...
              {currentImages.map((img, idx) => (
                  <Image
                      ...
                      className={`object-cover rounded cursor-pointer ${
                          idx === currentImageIndex ? 'border-2 border-[#26735B]' : 'border border-gray-200'
                      }`}
